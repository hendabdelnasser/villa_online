...
@Component({
  selector: 'app-consulted',
  templateUrl: './consulted.component.html',
  styleUrls: ['./consulted.component.scss']
})
export class ConsultedComponent implements OnInit {
  ...
  getProjects(){
    this._dashService.get(`/services/app/HomePanal/GetAllConsulted?tenantId=1`).subscribe(
      (res=>{
        this.isLoaded=true
        //@ts-ignore
        if(res['success']){
    ...
    this._dashService.post(`/services/app/HomePanal/CreateConsulted`,fetch).subscribe(
      (res=>{
        //@ts-ignore
        if(res['success']){
         this.isLoaded=true;
    ...
    deleteService(id:number,index:number){
      ...
      this._dashService.delete(`/services/app/HomePanal/DeleteConsulted?Id=${id}`).subscribe((response) => {
        //@ts-ignore
        if(response['success']){
      this.isLoaded=true;
